...
calc_surv_aah_gs <- nimble::nimbleFunction(
    run = function(
...
# for age 2+
    for(i in 2:n_agef) {
        for(j in 1:n_year) {
			cll_uch <- beta0 +
                       rep(age_effect[1:yr_end[i]],times=yr_end[j])+rep(period_effect[1:yr_end[j]],each=yr_end[i])
...
Ccalc_surv_aah_gs <- compileNimble(calc_surv_aah_gs)

starttime <- Sys.time()
sn_sus_gs <- Ccalc_surv_aah_gs(
    #nT_age = nT_age_surv,
...
calc_surv_harvest <- nimble::nimbleFunction(
	...
	for(i in 1:n_year){
		p_vec_f[yr_start[i]:yr_end[i]] <- c(rep(1,ng_start[i]-yr_start[i]),rep(p_nogun_f,gun_start[i]-ng_start[i]),p_gun_f,rep(p_nogun_f,ng_end[i]-gun_end[i]),rep(1,yr_end[i]-ng_end[i]))
			
		p_vec_m[yr_start[i]:yr_end[i]] <- c(rep(1,ng_start[i]-yr_start[i]),rep(p_nogun_m,gun_start[i]-ng_start[i]),p_gun_m,rep(p_nogun_m,ng_end[i]-gun_end[i]),rep(1,yr_end[i]-ng_end[i]))
	}
	
# for age 1
        for(j in 1:n_year) {
			l_uch <- beta0 + rep(age_effect[yr_start[1]:yr_end[1]],times=intvl_step_yr)+rep(period_effect[yr_start[j]:yr_end[j]],each=intvl_step_yr)

			l_uch_hunt <- beta0 + rep(age_effect[yr_start[1]:ng_end[1]],times=ng_end[j]+1-yr_start[j])+rep(period_effect[yr_start[j]:ng_end[j]],each=ng_end[1])

            S0[1,1,j] <- exp(-sum(exp(l_uch + beta_sex)))
            S0[2,1,j] <- exp(-sum(exp(l_uch)))
            S0_hunt[1,1,j] <- exp(-sum(exp(l_uch_hunt + beta_sex)*rep(p_vec_f[yr_start[j]:ng_end[j]],each=ng_end[1])))
            S0_hunt[2,1,j] <- exp(-sum(exp(l_uch_hunt)*rep(p_vec_m[yr_start[j]:ng_end[j]],each=ng_end[1])))
            
			S0_hunt2[1,1,j] <- exp(-sum(exp(l_uch + beta_sex)*rep(p_vec_f[yr_start[j]:yr_end[j]],each=yr_end[1])))
            S0_hunt2[2,1,j] <- exp(-sum(exp(l_uch)*rep(p_vec_m[yr_start[j]:yr_end[j]],each=yr_end[1])))
       }

# for age 2+
    for(i in 2:n_agef) {
        for(j in 1:n_year) {
			l_uch <- beta0 + rep(age_effect[1:yr_end[i]],times=yr_end[j])+rep(period_effect[1:yr_end[j]],each=yr_end[i])

			l_uch_hunt <- beta0 + rep(age_effect[1:ng_end[i]],times=ng_end[j])+rep(period_effect[1:ng_end[j]],each=ng_end[i])

            S0[1,i,j] <- exp(-sum(exp(l_uch + beta_sex)))
            S0[2,i,j] <- exp(-sum(exp(l_uch)))
            S0_hunt[1,i,j] <- exp(-sum(exp(l_uch_hunt + beta_sex)*rep(p_vec_f[1:ng_end[j]],each=ng_end[i])))
            S0_hunt[2,i,j] <- exp(-sum(exp(l_uch_hunt)*rep(p_vec_m[1:ng_end[j]],each=ng_end[i])))

			S0_hunt2[1,i,j] <- exp(-sum(exp(l_uch + beta_sex)*rep(p_vec_f[1:yr_end[j]],each=yr_end[i])))
            S0_hunt2[2,i,j] <- exp(-sum(exp(l_uch)*rep(p_vec_m[1:yr_end[j]],each=yr_end[i])))
       }
	}
	...
	# age > 2
        for (a in 3:n_agef) {
            ...
            #                    S0[1,a-1,t-1]
            s_hunt[1,a,t] <- exp(log(S0_hunt[1,a,t])+log(S0[1,a-1,t-1])-log(S0_hunt2[1,a-1,t-1]))/S0[1,a-1,t-1]
       }
        for(a in 3:n_agem) {
            #s_hunt[2,a,t] <- S0_hunt[2,a,t]/
            #                    S0[2,a-1,t-1]
            s_hunt[2,a,t] <- exp(log(S0_hunt[2,a,t])+log(S0[2,a-1,t-1])-log(S0_hunt2[2,a-1,t-1]))/S0[2,a-1,t-1]
       }
    }
  returnType(double(3))
