...
plot(sn_sus_fast[1,10,])
plot(sn_sus_fast[1,1,])
plot(s_aah[1,1,])
plot(s_aah[1,2,])
plot(s_aah[1,4,])
plot(s_aah[1,10,])
...
calc_surv_harvest <- nimble::nimbleFunction(
	...
	for(i in 1:n_year){
		p_vec_f[yr_start[i]:yr_end[i]] <- c(rep(1,ng_start[i]-yr_start[i]),rep(p_nogun_f,gun_start[i]-ng_start[i]),rep(p_gun_f,length(gun_start[i]:gun_end[i])),rep(p_nogun_f,ng_end[i]-gun_end[i]),rep(1,yr_end[i]-ng_end[i]))

		p_vec_m[yr_start[i]:yr_end[i]] <- c(rep(1,ng_start[i]-yr_start[i]),rep(p_nogun_m,gun_start[i]-ng_start[i]),rep(p_gun_m,length(gun_start[i]:gun_end[i])),rep(p_nogun_m,ng_end[i]-gun_end[i]),rep(1,yr_end[i]-ng_end[i]))
	}
...
# for age 1
        for(j in 1:n_year) {
            ...
            S0[1,1,j] <- sum(exp(l_uch + beta_sex))
            S0[2,1,j] <- sum(exp(l_uch))
            S0_hunt[1,1,j] <- sum(exp(l_uch_hunt + beta_sex)*rep(p_vec_f[yr_start[j]:ng_end[j]],each=ng_end[1]))
            S0_hunt[2,1,j] <- sum(exp(l_uch_hunt)*rep(p_vec_m[yr_start[j]:ng_end[j]],each=ng_end[1]))
            
			S0_hunt2[1,1,j] <- sum(exp(l_uch + beta_sex)*rep(p_vec_f[yr_start[j]:yr_end[j]],each=yr_end[1]))
            S0_hunt2[2,1,j] <- sum(exp(l_uch)*rep(p_vec_m[yr_start[j]:yr_end[j]],each=yr_end[1]))
       }
...
# for age 2+
    for(i in 2:n_agef) {
        for(j in 1:n_year) {
            ...
            S0[1,i,j] <- sum(exp(l_uch + beta_sex))
            S0[2,i,j] <- sum(exp(l_uch))
            S0_hunt[1,i,j] <- sum(exp(l_uch_hunt + beta_sex)*rep(p_vec_f[1:ng_end[j]],each=ng_end[i]))
            S0_hunt[2,i,j] <- sum(exp(l_uch_hunt)*rep(p_vec_m[1:ng_end[j]],each=ng_end[i]))

			S0_hunt2[1,i,j] <- sum(exp(l_uch + beta_sex)*rep(p_vec_f[1:yr_end[j]],each=yr_end[i]))
            S0_hunt2[2,i,j] <- sum(exp(l_uch)*rep(p_vec_m[1:yr_end[j]],each=yr_end[i]))
       }
	}
    ...
    # s_aah[sex, age(years), period(years)]
    for (t in 1:n_year) {
        s_hunt[1,1,t] <- exp(-S0_hunt[1,1,t]) #antlerless fawns
        s_hunt[2,1,t] <- exp(-S0_hunt[2,1,t]) #antlered fawns
    }
    # first year across all ages from sex-specific process model
    # antlerless yearlings and older first year
    for(a in 2:n_agef) {
